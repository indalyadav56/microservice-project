...
var u = ({ plugins: l2 = [] }) => {
  ...
  return l2.forEach((n) => {
    ...
    getViewComponents: (n) => Array.from(a.values()).flatMap((r) => {
      var t2;
      return ((t2 = r.views) == null ? void 0 : t2[n]) || [];
    }),
    /**
     * Execute a hook for a specific event
     */
    executeHook: (n, ...r) => {
      const t2 = Array.from(a.values()).flatMap(
        (e) => {
          var s2;
          return ((s2 = e.hooks) == null ? void 0 : s2[n]) || [];
        }
      );
      return Promise.all(
        t2.filter((e) => e != null).map((e) => e == null ? void 0 : e(...r))
      );
    }
  };
};
...
function C({
  ...
  return e.forEach((s2) => {
    const { name: c, value: u2, domain: m2, ...d } = s2;
    !k(o, m2) || !c || n.push(
      cookieSchema.parse({
        name: c,
        value: u2,
...
var $ = (l2 = [], i2 = {}, s2 = "") => {
  ...
  return l2.forEach((a) => {
    ...
    if (a.type === "http")
      if (a.scheme === "basic") {
        const o = p(a.username, i2), u2 = p(a.password, i2), r = `${o}:${u2}`;
        t2.Authorization = `Basic ${r === ":" ? "username:password" : btoa(r)}`;
      } else {
        const o = p(a.token, i2);
        t2.Authorization = `Bearer ${o || s2}`;
      }
    if (a.type === "oauth2") {
      const u2 = (p2 = Object.values(a.flows).filter(isDefined).find((r) => r.token)) == null ? void 0 : p2.token;
      t2.Authorization = `Bearer ${u2 || s2}`;
    }
...
var H = defineComponent({
  ...
  setup(b, { emit: h }) {
    ...
    return (e, u2) => e.isEditable ? (openBlock(), createBlock(unref(D), {
      key: 0,
      modelValue: i2.value,
      "onUpdate:modelValue": u2[0] || (u2[0] = (S) => i2.value = S),
...
//# sourceMappingURL=chunk-KEMKBPRU.js.map
